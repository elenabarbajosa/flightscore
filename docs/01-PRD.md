...
3. Recalculate the ranking in under 300 ms when the user changes a weight, without calling the flight API again.
...
- The product does not make bookings or process payments. It redirects the user to the airline or OTA.
...
- Airport autocomplete from a **local/static airport dataset**.
- Call to a flight-data API and normalization of results (EUR pricing via provider where supported, layover metadata, connection classification).
...
- Outbound link to the corresponding airline or OTA.
...
|---|---|---|---|
| Free-tier API quota limits | High | Medium | Cache by origin, destination, departure date, return date (`"ONE_WAY"` when absent), passengers, and cabin class (30 min TTL); quota counter and hard usage controls |
| API prices differ from the airline website | Medium | Medium | Label prices as "estimated"; provide a direct link for final confirmation |
| Incomplete airline / low-cost coverage | Medium | Medium | Document the limitation; keep the provider layer replaceable and evaluate complementary sources if needed |
...
| API key leakage | Low | High | All provider calls go through the server; credentials never appear in client code |
| Provider dependency | Medium | High | Keep all provider-specific logic isolated behind a provider abstraction |

---

## 8. External dependencies

- Developer account and credentials for the **flight-data provider selected during implementation**.
...
**Provider policy:** no flight-data provider is hard-coded as a permanent dependency. Provider-specific logic must remain isolated so the provider can be replaced without changing the scoring engine or UI.
...
1. A real long-haul search returns results ordered by score in under 5 seconds.
2. Changing any weight or preset reorders the list without a new flight API request.
...
5. Flight-provider credentials are not accessible from the browser (verified in client source and the network panel).
