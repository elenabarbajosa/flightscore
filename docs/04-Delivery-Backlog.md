...
|---|---|---:|---|
| **F0 — Foundation** | Repository, Next.js + TypeScript + Tailwind skeleton, documentation, mock-data setup, static airport dataset, empty deployment, SerpApi provider decision recorded | 2–3 days | Application skeleton deployed at the test URL |
| **F1 — Functional backend** | `/api/search`, SerpApi Google Flights integration, normalization, caching (`lib/cache.ts`), EUR handling | 5–7 days | A real SerpApi call returns JSON matching the internal contract |
...
| T-05 | Document SerpApi Google Flights configuration and obtain API credentials | F0 | — |
| T-06 | Implement provider abstraction and SerpApi Google Flights adapter (`lib/provider/serpapi.ts`) | F1 | — |
| T-07 | Implement `/api/search` with input validation and server-side SerpApi orchestration | F1 | US-01 |
| T-08 | Normalize SerpApi responses into the internal contract (EUR via SerpApi, layovers, `connectionType`) | F1 | US-05 |
...
| T-13 | Add unit tests for normalization (`lib/normalize.ts`) using SerpApi fixtures/mocks | F2 | — |
...
- [ ] No credentials, secret keys, SerpApi payloads, or internal provider details are exposed in client-side code.
...
| P-02 | Change Price weight from 5 to 9 | Instant reordering, no SerpApi request |
...
| P-05 | Simulate SerpApi failure | Readable English message and retry option |
| P-06 | Inspect client source and browser network panel | No SerpApi credentials visible |
...
11. The MVP flight-data provider is **SerpApi using the Google Flights API**. Provider-specific logic must remain replaceable through the provider abstraction.
12. Airport autocomplete uses a **local/static dataset** (min. fields: iata, name, city, countryCode, latitude, longitude), not SerpApi.
...
15. Request EUR from SerpApi / Google Flights whenever supported; use provider currency conversion when available; no separate FX service unless SerpApi requires it.
...
24. All SerpApi calls are server-side. Raw SerpApi responses are never passed to the UI or scoring engine.
25. Weight changes must never trigger a new SerpApi search.
26. FlightScore does not sell tickets, process bookings, or handle flight payments; users are redirected to the airline or OTA.
